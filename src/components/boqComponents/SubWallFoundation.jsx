...
//  useEffect to get data from database
    ...
    try {
      if (savedInfo.savedPreId) {
        const response = await axios.get(
          API_BASE_URL + `/api/savedwallfoundations/${savedInfo.savedPreId}`,config
        );
  ...
  const handleQuantityUpdate = async (materialId) => {
    if (quantity !== "") {
      try {
        const response = await axios.put(
          API_BASE_URL + `/api/savedwallfoundations/${savedInfo.savedPreId}`,
          {
  ...
  const totalAmount = savedData?.wallData.reduce((total, data) => {
    const material = wallRows.find((row) => row._id === data.materialId);
    if (material) {
...
export default SubWallFoundation;

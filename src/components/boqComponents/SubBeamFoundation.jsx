...
//  useEffect to get data from database
  ...
  const totalAmount = savedData?.beamData.reduce((total, data) => {
    const material = groundbeamRows.find((row) => row._id === data.materialId);
    if (material) {
          ...
          <TableHead>
            ...
            {groundbeamRows.map((row) => (
              ...
              >
                ...
                <StyledTableCell align="right">{row.unit}</StyledTableCell>
                <StyledTableCell align="right">
                  {savedData?.beamData !== null ? (
                    <Box>
          
                      {savedData?.beamData.map((data) => {
...
export default SubBeamFoundation;

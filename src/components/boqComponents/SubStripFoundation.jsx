...
//  useEffect to get data from database
  ...
  const totalAmount = savedData?.stripData.reduce((total, data) => {
    const material = stripRows.find((row) => row._id === data.materialId);
    if (material) {
          ...
          <TableHead>
            ...
            {stripRows.map((row) => (
              ...
              >
                ...
                <StyledTableCell align="right">{row.unit}</StyledTableCell>
                <StyledTableCell align="right">
                  {savedData?.stripData !== null ? (
                    <Box>
          
                      {savedData?.stripData.map((data) => {
...
export default SubStripFoundation;

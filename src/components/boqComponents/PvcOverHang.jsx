...
const PvcOverHang = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(API_BASE_URL + "/api/pvc",config)
      .then((response) => {
  ...
  const fetchUpdatedData = async () => {
    try {
      const response = await axios.get(API_BASE_URL + "/api/pvc",config);
      setPvcs(response.data);
    } catch (error) {
    ...
    if (newRate !== null) {
      try {
        const response = await axios.put(
          API_BASE_URL + `/api/pvc/${materialId}`,
...
const fetchSavedData = async () => {
    try {
      if (savedInfo.savedPreId) {
        const response = await axios.get(
          API_BASE_URL + `/api/savedpvcs/${savedInfo.savedPreId}`,config
        );
  ...
  const handleQuantityUpdate = async (materialId) => {
    if (quantity !== "") {
      try {
        const response = await axios.put(
          API_BASE_URL + `/api/savedpvcs/${savedInfo.savedPreId}`,
          {
  ...
  const totalAmount = savedData?.pvcData.reduce((total, data) => {
    const material = pvcs.find((row) => row._id === data.materialId);
    if (material) {
  ...
  return (
   ...
   <StyledTableCell align="center">
                  {savedData?.pvcData !== null ? (
                    <Box>
          
                      {savedData?.pvcData.map((data) => {

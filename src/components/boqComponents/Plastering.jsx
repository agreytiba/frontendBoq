...
const Plastering = () => {
  ...
  const [plasteringRows, setPlasteringRows] = useState([]);
  // edit rate useState
 ...
 // use state to get savedpre data
  ...
  const totalAmount = savedData?.plasterData.reduce((total, data) => {
    const material = plasteringRows.find((row) => row._id === data.materialId);
    if (material) {
          ...
          <TableBody>
            {plasteringRows.map((row) => (
              ...
              <StyledTableCell align="center">
                  {savedData?.plasterData !== null ? (
                    <Box>
          
                      {savedData?.plasterData.map((data) => {

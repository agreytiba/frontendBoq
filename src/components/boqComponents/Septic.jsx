...
const savedInfo = JSON.parse(localStorage.getItem("savedData"));
    ...
    try {
      ...
      if (savedInfo.savedPreId) {
        const response = await axios.get(
          API_BASE_URL + `/api/savedseptic/${savedInfo.savedPreId}`,
          config
        );
  ...
  const handleQuantityUpdate = async (materialId) => {
    if (quantity !== "") {
      try {
        const response = await axios.put(
          API_BASE_URL + `/api/savedseptic/${savedData._id}`,
          {
  ...
  const totalAmount = savedData?.septicData.reduce((total, data) => {
              ...
              <TableBody>
                {dataRows?.map((row) => (
                  ...
                  >
                    ...
                    <StyledTableCell align="right">{row.unit}</StyledTableCell>
                    <StyledTableCell align="right">
                      {savedData?.septicData !== null ? (
                        <Box>
                          {savedData?.septicData.map((data) => {

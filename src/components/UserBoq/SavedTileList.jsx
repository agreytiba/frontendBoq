...
const SavedTileList = () => {
  ...
  useEffect(() => {
    const fetchSavedPres = async () => {
      try {
        const response = await axios.get('https://backendboq.onrender.com/api/savedtiles');
  ...
  const columns = [
    ...
    renderCell: (params) => (
      <Button
        variant="outlined"
        onClick={() => handleViewPreData(params.row.tileData)}
      >
        Tiles
      </Button>
    ),
  ...
  const handleViewPreData = (tileData) => {
  setSelectedPreData(tileData);
  setOpenDialog(true);
};
  ...
  return (
    <Box display={"flex"} justifyContent={"center"} alignItems={"center"}>
        ...
        <Dialog open={openDialog} onClose={handleCloseDialog}>
          <DialogTitle style={{textAlign:"center",backgroundColor:"goldenrod",color:"#fff"}}>Tiles Material</DialogTitle>
          <DialogContent>
            <TableContainer>
              <Table>
                <TableHead>

...
const SavedElectricalList = () => {
  ...
  useEffect(() => {
    const fetchSavedPres = async () => {
      try {
        const response = await axios.get('https://backendboq.onrender.com/api/savedelectrical');
  ...
  const columns = [
    ...
    renderCell: (params) => (
      <Button
        variant="outlined"
        onClick={() => handleViewPreData(params.row.electricData)}
      >
    Plastering
      </Button>
    ),
  ...
  const handleViewPreData = (electricData) => {
  setSelectedPreData(electricData);
  setOpenDialog(true);
};
...
export default SavedElectricalList;

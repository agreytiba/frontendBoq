...
const SavedWallingList = () => {
  ...
  useEffect(() => {
    const fetchSavedPres = async () => {
      try {
        const response = await axios.get('https://backendboq.onrender.com/api/savedwalling');
  ...
  const columns = [
    ...
    renderCell: (params) => (
      <Button
        variant="outlined"
        onClick={() => handleViewPreData(params.row.wallData)}
      >
        Walling
      </Button>
    ),
  ...
  const handleViewPreData = (wallData) => {
  setSelectedPreData(wallData);
  setOpenDialog(true);
};
const handleCloseDialog = () => {
  ...
  return (
    <Box display={"flex"} justifyContent={"center"} alignItems={"center"}>
          <Box width={"80%"} border={"1px solid #333"} p="10px" my={"10px"}   sx={{
          "& .MuiDataGrid-footerContainer": {
            ...
            display: "none",
              ...
              <Typography variant='h2'>walling</Typography>
        <DataGrid rows={savedData} columns={columns} pageSize={5} getRowId={getRowId} />
     
        <Dialog open={openDialog} onClose={handleCloseDialog}>
          <DialogTitle style={{textAlign:"center",backgroundColor:"goldenrod",color:"#fff"}}>Walling material</DialogTitle>
          <DialogContent>
            <TableContainer>
              <Table>
                <TableHead>
...
export default SavedWallingList;

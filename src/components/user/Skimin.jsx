...
const Skimin = () => {
  ...
  useEffect(() => {
    const fetchSavedPres = async () => {
      try {
        const response = await axios.get('https://backendboq.onrender.com/api/savedskiminside"');
  ...
  const columns = [
    {
      ...
      renderCell: (params) => (
        <Button
          variant="outlined"
          onClick={() => handleViewPreData(params.row.skimData)}
        >
           Wall Skimming inside
        </Button>
      ),
    },
  ];

  const handleViewPreData = (skimData) => {
    setSelectedPreData(skimData);
    setOpenDialog(true);
  };
  ...
  const handleDownloadPDF = () => {
    ...
    doc.text('Wall  Skimming inside', 10, 10);
      ...
      <Box width={"80%"} sx={{
        ...
        <DataGrid rows={savedPres} columns={columns} pageSize={5} getRowId={getRowId} />
              ...
              <Dialog open={openDialog} >
                  ...
                  }}>Wall Skimming  inside</DialogTitle>
          <DialogContent>
            <TableContainer>
              <Table>
                <TableHead>

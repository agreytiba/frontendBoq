...
const Grills = () => {
  ...
  useEffect(() => {
    const fetchSavedPres = async () => {
      try {
        const response = await axios.get('https://backendboq.onrender.com/api/savedgrills');
  ...
  const columns = [
    {
      ...
      renderCell: (params) => (
        <Button
          variant="outlined"
          onClick={() => handleViewPreData(params.row.grillData)}
        >
          window grills
        </Button>
      ),
  ...
  const handleViewPreData = (grillData) => {
    setSelectedPreData(grillData);
    setOpenDialog(true);
  };

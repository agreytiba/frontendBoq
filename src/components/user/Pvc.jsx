...
const [savedPres, setSavedPres] = useState([]);
  ...
  const columns = [
    ...
    renderCell: (params) => (
      <Button
        variant="outlined"
        onClick={() => handleViewPreData(params.row.pvcData)}
      >
        Pvcs
      </Button>
    ),
  ...
  const handleViewPreData = (pvcData) => {
  setSelectedPreData(pvcData);
  setOpenDialog(true);
};
...
const handleDownloadPDF = () => {
  ...
  <Box width={"80%"}   sx={{
        ...
        <Dialog open={openDialog} onClose={handleCloseDialog}>
          <DialogTitle style={{textAlign:"center",backgroundColor:"goldenrod",color:"#fff"}}>Pvcs</DialogTitle>
          <DialogContent>
            <TableContainer>
              <Table>
                <TableHead>

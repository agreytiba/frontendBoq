...
const [savedPres, setSavedPres] = useState([]);
  ...
  useEffect(() => {
    const fetchSavedPres = async () => {
      try {
        const response = await axios.get('https://backendboq.onrender.com/api/savedroofing');
  ...
  const columns = [
    ...
    renderCell: (params) => (
      <Button
        variant="outlined"
        onClick={() => handleViewPreData(params.row.roofData)}
      >
    Roofing
      </Button>
    ),
  ...
  const handleViewPreData = (roofData) => {
  setSelectedPreData(roofData);
  setOpenDialog(true);
};
...
const handleDownloadPDF = () => {
    ...
    doc.text('Roofing Material', 10, 10);
  ...
  <Box width={"80%"}   sx={{
        ...
        <Dialog open={openDialog} onClose={handleCloseDialog}>
          <DialogTitle style={{textAlign:"center",backgroundColor:"goldenrod",color:"#fff"}}>Roofing Materials</DialogTitle>
          <DialogContent>
            <TableContainer>
              <Table>
                <TableHead>

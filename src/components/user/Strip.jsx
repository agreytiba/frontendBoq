...
const Strip = () => {
  ...
  useEffect(() => {
    const fetchSavedPres = async () => {
      try {
        const response = await axios.get('https://backendboq.onrender.com/api/savedStrips');
  ...
  const columns = [
    {
      ...
      renderCell: (params) => (
        <Button
          variant="outlined"
          onClick={() => handleViewPreData(params.row.stripData)}
        >
          Strip foundation
        </Button>
      ),
  ...
  const handleViewPreData = (stripData) => {
    setSelectedPreData(stripData);
    setOpenDialog(true);
  };
  ...
  const handleDownloadPDF = () => {
    ...
    doc.text('Substructure Strip Foundation', 10, 10);
      ...
      <Box width={"80%"} sx={{
        ...
        <DataGrid rows={savedPres} columns={columns} pageSize={5} getRowId={getRowId} />
              ...
              <Dialog open={openDialog} >
                  ...
                  }}>Substructure Strip Foundation</DialogTitle>
          <DialogContent>
            <TableContainer>
              <Table>
                <TableHead>

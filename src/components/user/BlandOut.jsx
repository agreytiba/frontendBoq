...
const Blandout = () => {
  ...
  useEffect(() => {
    const fetchSavedPres = async () => {
      try {
        const response = await axios.get('https://backendboq.onrender.com/api/savedblandoutside');
  ...
  const columns = [
    {
      ...
      renderCell: (params) => (
        <Button
          variant="outlined"
          onClick={() => handleViewPreData(params.row.insideData)}
        >
          Blandering Inside
        </Button>
      ),
  ...
  const handleViewPreData = (insideData) => {
    setSelectedPreData(insideData);
    setOpenDialog(true);
  };
  ...
  const handleDownloadPDF = () => {
    ...
    doc.text('Substructure Blandering Inside', 10, 10);
    ...
    doc.save('BlanderingOutside.pdf');
  };
      ...
      <Box width={"80%"} sx={{
        ...
        <DataGrid rows={savedPres} columns={columns} pageSize={5} getRowId={getRowId} />
              ...
              <Dialog open={openDialog} >
                  ...
                  }}>Substructure Blandering Outside</DialogTitle>
          <DialogContent>
            <TableContainer>
              <Table>
                <TableHead>

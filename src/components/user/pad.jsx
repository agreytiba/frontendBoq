...
const Pad = () => {
  ...
  useEffect(() => {
    const fetchSavedPres = async () => {
      try {
        const response = await axios.get('https://backendboq.onrender.com/api/savedpads');
  ...
  const columns = [
    {
      ...
      renderCell: (params) => (
        <Button
          variant="outlined"
          onClick={() => handleViewPreData(params.row.padData)}
        >
          pad foundation
        </Button>
      ),
  ...
  const handleViewPreData = (padData) => {
    setSelectedPreData(padData);
    setOpenDialog(true);
  };
  ...
  const handleDownloadPDF = () => {
    ...
    doc.text('Substructure pad foundations', 10, 10);
      ...
      <Box width={"80%"} sx={{
        ...
        <DataGrid rows={savedPres} columns={columns} pageSize={5} getRowId={getRowId} />
              ...
              <Dialog open={openDialog} >
                  ...
                  }}>Substructure pad foundation and columns</DialogTitle>
          <DialogContent>
            <TableContainer>
              <Table>
                <TableHead>

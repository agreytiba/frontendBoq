...
const Beams = () => {
  ...
  useEffect(() => {
    const fetchSavedPres = async () => {
      try {
        const response = await axios.get('https://backendboq.onrender.com/api/savedbeams');
  ...
  const columns = [
    {
      ...
      renderCell: (params) => (
        <Button
          variant="outlined"
          onClick={() => handleViewPreData(params.row.beamData)}
        >
          Beams foundation
        </Button>
      ),
  ...
  const handleViewPreData = (beamData) => {
    setSelectedPreData(beamData);
    setOpenDialog(true);
  };
  ...
  const handleDownloadPDF = () => {
    ...
    doc.text('Substructure Ground Beams', 10, 10);
      ...
      <Box width={"80%"} sx={{
        ...
        <DataGrid rows={savedPres} columns={columns} pageSize={5} getRowId={getRowId} />
              ...
              <Dialog open={openDialog} >
                  ...
                  }}>Substructure  ground beams</DialogTitle>
          <DialogContent>
            <TableContainer>
              <Table>
                <TableHead>

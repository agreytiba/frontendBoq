...
const Frame = () => {
  ...
  useEffect(() => {
    const fetchSavedPres = async () => {
      try {
        const response = await axios.get('https://backendboq.onrender.com/api/savedframes');
  ...
  const columns = [
    {
      ...
      renderCell: (params) => (
        <Button
          variant="outlined"
          onClick={() => handleViewPreData(params.row.frameData)}
        >
          Door Frames
        </Button>
      ),
  ...
  const handleViewPreData = (frameData) => {
    setSelectedPreData(frameData);
    setOpenDialog(true);
  };

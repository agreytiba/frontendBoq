...
const [savedPres, setSavedPres] = useState([]);
  ...
  useEffect(() => {
    const fetchSavedPres = async () => {
      try {
        const response = await axios.get('https://backendboq.onrender.com/api/savedgypsum');
  ...
  const columns = [
    ...
    renderCell: (params) => (
      <Button
        variant="outlined"
        onClick={() => handleViewPreData(params.row.gypsumData)}
      >
       Gypsum
      </Button>
    ),
  ...
  const handleViewPreData = (gypsumData) => {
  setSelectedPreData(gypsumData);
  setOpenDialog(true);
};
const handleCloseDialog = () => {
 ...
 const handleDownloadPDF = () => {
    ...
    doc.text('Gysum Ceiling Material', 10, 10);
  ...
  <Box width={"80%"}   sx={{
        ...
        <Dialog open={openDialog}>
          <DialogTitle style={{textAlign:"center",backgroundColor:"goldenrod",color:"#fff"}}>Gypsum ceiling  material</DialogTitle>
          <DialogContent>
            <TableContainer>
              <Table>
                <TableHead>

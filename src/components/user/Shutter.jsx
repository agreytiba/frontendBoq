...
const Shutter = () => {
  ...
  useEffect(() => {
    const fetchSavedPres = async () => {
      try {
        const response = await axios.get('https://backendboq.onrender.com/api/savedshutters');
  ...
  const columns = [
    {
      ...
      renderCell: (params) => (
        <Button
          variant="outlined"
          onClick={() => handleViewPreData(params.row.shutterData)}
        >
          Door Shutter
        </Button>
      ),
  ...
  const handleViewPreData = (shutterData) => {
    setSelectedPreData(shutterData);
    setOpenDialog(true);
  };

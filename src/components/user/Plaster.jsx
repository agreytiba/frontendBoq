...
const [savedPres, setSavedPres] = useState([]);
  ...
  useEffect(() => {
    const fetchSavedPres = async () => {
      try {
        const response = await axios.get(API_BASE_URL + '/api/savedplastering',config);
...
const handleCloseDialog = () => {
  ...
  setOpenDialog(false);
    ...
    doc.text('plastering materials', 10, 10);
  ...
  <Box width={"80%"}   sx={{
        ...
        <Dialog open={openDialog}>
          <DialogTitle style={{textAlign:"center",backgroundColor:"goldenrod",color:"#fff"}}>Plastering Materials</DialogTitle>
          <DialogContent>
            <TableContainer>
              <Table>
                <TableHead>
